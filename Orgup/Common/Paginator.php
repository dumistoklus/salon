...
class Paginator {
	...
	function __construct($itemscount, $currentpage = 1, $limit = 20, $mid_range = 6 ) {
	...
	private function calculateRange() {
		...
		$this->start_range = (int)$this->currentpage - floor( $this->mid_range/2 );
		$this->end_range = (int)$this->currentpage + floor( $this->mid_range/2 );
        ...
        // fix secure bug
        if ( $this->start_range > $this->numpages ) {
            $this->start_range = (int)$this->numpages - floor( $this->numpages/2 );
        }
	...
	public function getStart_range() {
